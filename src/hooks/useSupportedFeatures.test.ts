...
describe('getSupportedFeatures', () => {
  ...
  describe('supportsRepeat', () => {
    ...
    it('should work with different repeat values', () => {
      ...
      expect(getSupportedFeatures('playing', { ...baseFeatures, repeat: 'one' }).supportsRepeat).toBe(true);
      expect(getSupportedFeatures('playing', { ...baseFeatures, repeat: 'all' }).supportsRepeat).toBe(true);
      expect(getSupportedFeatures('playing', { ...baseFeatures, repeat: 'off' }).supportsRepeat).toBe(true);
    });
  });
  ...
  describe('feature bit validation', () => {
    it('should validate shuffle feature bit (32768)', () => {
      ...
      expect(getSupportedFeatures('playing', { ...shuffleAttrs, supported_features: 32768 }).supportsShuffle).toBe(true);
      expect(getSupportedFeatures('playing', { ...shuffleAttrs, supported_features: 32767 }).supportsShuffle).toBe(false);
      expect(getSupportedFeatures('playing', { ...shuffleAttrs, supported_features: 65536 }).supportsShuffle).toBe(false);
    });
    ...
    it('should validate repeat feature bit (262144)', () => {
      ...
      expect(getSupportedFeatures('playing', { ...repeatAttrs, supported_features: 262144 }).supportsRepeat).toBe(true);
      expect(getSupportedFeatures('playing', { ...repeatAttrs, supported_features: 262143 }).supportsRepeat).toBe(false);
      expect(getSupportedFeatures('playing', { ...repeatAttrs, supported_features: 131072 }).supportsRepeat).toBe(false);
    });

    it('should work with combined feature flags', () => {
      const combinedFeatures = 16 | 32 | 4096 | 16384 | 32768 | 262144; // All features
      const allAttrs = { shuffle: false, repeat: 'off', source: 'spotify', supported_features: combinedFeatures };
